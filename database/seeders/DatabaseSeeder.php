...
use App\Models\Branch;use App\Models\Category;use App\Models\MenuItem;use App\Models\Restaurant;use App\Models\User;use Illuminate\Database\Seeder;
class DatabaseSeeder extends Seeder
{
 public function run():void{
  ...
  foreach($items as [$cat,$en,$ar,$price,$order]){$item=MenuItem::updateOrCreate(['category_id'=>$cat->id,'name_en'=>$en],['name_ar'=>$ar,'price'=>$price,'is_available'=>true,'sort_order'=>$order]);$item->branches()->syncWithoutDetaching([$branch->id=>['is_available'=>true]]);}
  $password=env('SEED_ADMIN_PASSWORD'); if($password){User::updateOrCreate(['email'=>env('SEED_ADMIN_EMAIL','admin@example.test')],['name'=>env('SEED_ADMIN_NAME','Demo Admin'),'password'=>$password,'is_active'=>true]);}
 }
}
